...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        folder_path = os.path.join(settings.BASE_DIR, kwargs.get('folderFiles'))
        textures_path = os.path.join(folder_path, 'textures')
        objects_path = os.path.join(folder_path, 'objects')
        materials_path = os.path.join(folder_path, 'materials')
        ...
        textures = [f for f in media_files if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
        objects = [f for f in media_files if f.lower().endswith('.obj')]
        materials = [f for f in media_files if f.lower().endswith('.mtl')]
        ...
        for base_name_id, files in file_pairs.items():
            ...
            if texture_file and object_file and material_file:
                ...
                object_path = os.path.join(folder_path, object_file)
                material_path = os.path.join(folder_path, material_file)
